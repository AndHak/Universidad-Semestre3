...
class Main(QMainWindow):
    ...
    def __init__(self):
        ...
        self.email_saved = os.path.join(self.basedir, 'datos', 'email_saved.pkl')
        self.pass_saved = os.path.join(self.basedir, 'datos', 'pass_saved.pkl')
        ...
        self.texto_email_guardado = ""
        self.texto_pass_guardada = ""
    ...
    def setup_login_ui(self):
        ...
        self.remind_me.setChecked(bool(self.texto_email_guardado and self.texto_pass_guardada))
        ...
        self.email_line_edit.returnPressed.connect(self.log_in_user)
        self.password_line_edit.returnPressed.connect(self.log_in_user)
    ...
    def setup_ui_register(self):
        ...
        # Conectar eventos returnPressed de los line edits al método verificar_registracion
        self.name_line_edit.returnPressed.connect(self.verificar_registracion)
        self.last_name_line_edit.returnPressed.connect(self.verificar_registracion)
        self.email_register_line_edit.returnPressed.connect(self.verificar_registracion)
        self.password_register_line_edit.returnPressed.connect(self.verificar_registracion)
        self.password_confirm_line_edit.returnPressed.connect(self.verificar_registracion)
...
#Register methods
    ...
    def registrar_usuario(self):
        ...
        self.email_register_line_edit.setText("")
        self.password_register_line_edit.setText("")
        self.name_line_edit.setText("")
        self.last_name_line_edit.setText("")
        self.password_confirm_line_edit.setText("")
...
#Log in methods
    ...
    def iniciar_sesion_succes(self):
        if self.remind_me.isChecked():
            with open(self.email_saved, 'wb') as file:
                pickle.dump(self.email_line_edit.text(), file)
            with open(self.pass_saved, 'wb') as file:
                pickle.dump(self.password_line_edit.text(), file)
        else:
            # Borrar archivos de email y contraseña guardados si no se quiere recordar
            if os.path.exists(self.email_saved):
                os.remove(self.email_saved)
            if os.path.exists(self.pass_saved):
                os.remove(self.pass_saved)

            
        message = "Inicio de sesión exitoso"
        self.mostrar_notificacion(message, estilo_notificacion_succes)
        self.dashboard = Dashboard(usuario_logueado=self.usuario_encontrado)
        self.close()
        self.dashboard.show()
...
#UI methods
    ...
    def cargar_datos_guardados(self):
        if os.path.exists(self.email_saved):
            with open(self.email_saved, 'rb') as file:
                self.texto_email_guardado = pickle.load(file)

        if os.path.exists(self.pass_saved):
            with open(self.pass_saved, 'rb') as file:
                self.texto_pass_guardada = pickle.load(file)

        # Si hay datos guardados, actualizar los campos y marcar la casilla
        if self.texto_email_guardado and self.texto_pass_guardada:
            self.email_line_edit.setText(self.texto_email_guardado)
            self.password_line_edit.setText(self.texto_pass_guardada)
...
#################################
#Open complete info and dashboard
    def open_complete_info(self):
        self.ui_iniciar_sesion.hide()
        self.ui_registrarse.hide()
        self.setup_ui.hide()
