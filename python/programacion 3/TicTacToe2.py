...
class MainWindow(QMainWindow):
    ...
    def verify_moves(self):
        # Determinar el conjunto de movimientos y el jugador actual
        player_moves = self.x_moves if self.current_player == "X" else self.o_moves
        player = self.label_player_one.text() if self.current_player == "X" else self.label_player_two.text()

        counters = {
            'rows': {0: 0, 1: 0, 2: 0},
            'cols': {0: 0, 1: 0, 2: 0},
            'diags': {'main': 0, 'secondary': 0}
        }
        ...
        for move in player_moves:
            ...
            if row == col:
                counters['diags']['main'] += 1

            if row + col == 2:
                counters['diags']['secondary'] += 1
        

        win_condition = 3
        if any(value == win_condition for value in counters['rows'].values()) or \
        any(value == win_condition for value in counters['cols'].values()) or \
        counters['diags']['main'] == win_condition or \
        counters['diags']['secondary'] == win_condition:
