...
# Configuración de PyGame para el manejo del audio
pygame.mixer.init()
...
class VentanaPrincipal(QMainWindow):
    def __init__(self, datos_audio, total_frames, tamaño_frame, num_barras, sensibilidad):
        super().__init__()
        self.setWindowTitle("Visualizador AM Modular")
...
if __name__ == "__main__":
    ruta_archivo = r'C:\Programacion Universidad\Semestre 3\python\programacion 3\Proyecto 2\canciones\Shape of my Heart.mp3'
    datos_audio, temp_wav_path = obtener_datos_audio(ruta_archivo)
    tamaño_frame = 2048
    total_frames = len(datos_audio) // tamaño_frame
    num_barras = 15
    sensibilidad = 1000 

    app = QApplication(sys.argv)
    ventana_principal = VentanaPrincipal(datos_audio, total_frames, tamaño_frame, num_barras, sensibilidad)
    ...
    sys.exit(app.exec())
    os.remove(temp_wav_path)  # Elimina el archivo WAV temporal después de la ejecución del programa
