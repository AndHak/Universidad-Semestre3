        ...
        if item_seleccionado:
            ...
            if checked:
                ...
                if nombre_cancion not in [self.favorite_song_list.item(i).text() for i in range(self.favorite_song_list.count())]:
                    ...
                    self.lista_de_favoritos.append(nombre_cancion)
                    self.estado_favoritos[nombre_cancion] = True
            else:
                # Eliminar de favoritos si está en la lista
                self.eliminar_cancion_de_lista(nombre_cancion, self.favorite_song_list)
                self.eliminar_cancion_de_lista(nombre_cancion, self.lista_de_favoritos)
                self.estado_favoritos[nombre_cancion] = False

        self.actualizar_estado_boton_favorito(lista_widget)

    def eliminar_cancion_de_lista(self, nombre_cancion, lista):
        for i in range(lista.count()):
            if lista.item(i).text() == nombre_cancion:
                lista.takeItem(i)
                return
